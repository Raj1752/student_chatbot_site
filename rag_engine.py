...
# 3. Exam-start questions are handled safely.
# 4. Semester 1 and Semester 2 start/end dates are read from the PDF.
# 5. Existing illness, ID-card, retrieval and fallback logic is kept.
# ============================================================
...
def get_rag_response(user_input):
    ...
    # --------------------------------------------------------
    # Precise semester-date questions
    # Read Semester 1 and Semester 2 dates directly from the PDF.
    # --------------------------------------------------------
    semester_1_dates = re.search(
        r"Semester\s*1\s+starts\s+on\s+"
        r"(.+?)\s+and\s+ends\s+on\s+(.+?)[.]",
        raw_text,
        re.IGNORECASE,
    )
    semester_2_dates = re.search(
        r"Semester\s*2\s+starts\s+on\s+"
        r"(.+?)\s+and\s+ends\s+on\s+(.+?)[.]",
        raw_text,
        re.IGNORECASE,
    )

    if "semester" in question and not exam_context:
        asks_start = contains_any(question, ["start", "begin"])
        asks_end = contains_any(question, ["end", "finish"])
        asks_dates = contains_any(question, ["date", "dates"])

        if "semester 1" in question and semester_1_dates:
            start_date = semester_1_dates.group(1).strip()
            end_date = semester_1_dates.group(2).strip()

            if asks_start:
                return f"Semester 1 starts on {start_date}."
            if asks_end:
                return f"Semester 1 ends on {end_date}."
            if asks_dates:
                return (
                    f"Semester 1 starts on {start_date} "
                    f"and ends on {end_date}."
                )

        if "semester 2" in question and semester_2_dates:
            start_date = semester_2_dates.group(1).strip()
            end_date = semester_2_dates.group(2).strip()

            if asks_start:
                return f"Semester 2 starts on {start_date}."
            if asks_end:
                return f"Semester 2 ends on {end_date}."
            if asks_dates:
                return (
                    f"Semester 2 starts on {start_date} "
                    f"and ends on {end_date}."
                )

        if semester_1_dates and semester_2_dates:
            semester_1_start = semester_1_dates.group(1).strip()
            semester_1_end = semester_1_dates.group(2).strip()
            semester_2_start = semester_2_dates.group(1).strip()
            semester_2_end = semester_2_dates.group(2).strip()

            if asks_start:
                return (
                    f"Semester 1 starts on {semester_1_start}, "
                    f"and Semester 2 starts on {semester_2_start}."
                )

            if asks_end:
                return (
                    f"Semester 1 ends on {semester_1_end}, "
                    f"and Semester 2 ends on {semester_2_end}."
                )

            if asks_dates:
                return (
                    f"Semester 1 starts on {semester_1_start} "
                    f"and ends on {semester_1_end}. "
                    f"Semester 2 starts on {semester_2_start} "
                    f"and ends on {semester_2_end}."
                )

        if asks_start or asks_end or asks_dates:
            return FALLBACK_RESPONSE
