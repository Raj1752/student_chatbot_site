...
def get_rag_response(user_input):
    ...
    if (
        exam_context
        and "start" in question
        and "semester" not in question
    ):
    ...
    # Do not confuse this with examination dates.
    # --------------------------------------------------------
    if (
        ...
        semester_start_answer = None

        for sentence in all_sentences:
            lower_sentence = sentence.lower()

            if (
                "semester" in lower_sentence
                and contains_any(lower_sentence, ["start", "starts", "begin", "begins"])
                and not contains_any(lower_sentence, ["exam", "examination"])
            ):
                semester_start_answer = sentence
                break

        return semester_start_answer or FALLBACK_RESPONSE
